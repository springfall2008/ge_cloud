...
async def async_setup_default_selects(hass: HomeAssistant, config, serial, async_add_entities):
    """
...
class CloudSelect(CoordinatorEntity[CloudCoordinator], SelectEntity):
    ...
    def current_option(self) -> str:
        ...
        _LOGGER.info(f"Getting current option for {self.entity_description.key} number {self.reg_number} returns {option}")
        return option
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        if option is not None:
            ...
            result = await self.coordinator.api.async_write_inverter_setting(self.serial, reg_number, option)
